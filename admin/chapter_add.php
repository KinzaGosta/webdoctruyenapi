...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $stmt = $conn->prepare("INSERT INTO novel_chapters (novel_id, title, slug, content, order_index) VALUES (?, ?, ?, ?, ?)");
    $stmt->bind_param("isssi", $novel_id, $title, $slug, $content, $order);
...
<div class="d-flex">
    ...
    <div class="container-fluid p-4">
        <h2 class="fw-bold mb-4">➕ Thêm Chương: <span class="text-primary"><?= htmlspecialchars($novel['title']) ?></span></h2>
                ...
                <form method="POST">
                    <div class="row">
                        <div class="col-md-3 mb-3">
                            ...
                            <input type="number" name="order_index" class="form-control" value="<?= $next_index ?>" required>
                        </div>
                        <div class="col-md-9 mb-3">
                            <label class="form-label fw-bold">Tên Chương</label>
                            <input type="text" name="title" class="form-control" placeholder="Vd: Chương 1: Mở đầu" required>
                        </div>
                    </div>
                    <div class="mb-3">
                        <label class="form-label fw-bold">Nội dung</label>
                        <textarea name="content" class="form-control" rows="15" placeholder="Nhập nội dung truyện..." required></textarea>
                    </div>
                    <div class="d-flex gap-2">
                        <button type="submit" class="btn btn-success px-4">Lưu Chương</button>

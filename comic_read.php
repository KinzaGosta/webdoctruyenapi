...
async function loadComicChapter() {
    ...
    if (resStory && resStory.status === 'success' && resStory.data) {
        ...
        document.getElementById('comic-link-display').href = `comic_detail.php?slug=${comicSlug}`;
        ...
        if(chapters.length > 0) {
            ...
            for (let i = 0; i < chapters.length; i++) {
                ...
                let oLink = `comic_read.php?api=${btoa(chapters[i].chapter_api_data)}&name=${oName}&slug=${story.slug}`;
            ...
            if (foundIndex !== -1) {
                ...
                if (chapters[nextIdx]) {
                    ...
                    let nLink = `comic_read.php?api=${btoa(chapters[nextIdx].chapter_api_data)}&name=${nName}&slug=${story.slug}`;
                ...
                if (chapters[prevIdx]) {
                    ...
                    let pLink = `comic_read.php?api=${btoa(chapters[prevIdx].chapter_api_data)}&name=${pName}&slug=${story.slug}`;

...
async function loadNovelDetail() {
    ...
    if (res && res.status === 'success' && res.data) {
        ...
        if (chapters.length > 0) {
            ...
            btnHtml += `<a href="novel_read.php?id=${firstChap.id}" class="btn btn-primary btn-lg px-4 shadow-sm"><i class="fas fa-book-open"></i> Đọc Từ Đầu (${firstChap.title})</a>`;
            btnHtml += `<a href="novel_read.php?id=${latestChap.id}" class="btn btn-danger btn-lg px-4 shadow-sm"><i class="fas fa-bolt"></i> Mới Nhất (${latestChap.title})</a>`;
        } else {
        ...
        <?php if(isset($_SESSION['user_id'])): ?>
            ...
            btnHtml += `<button id="btn-favorite" class="btn btn-lg btn-outline-warning text-dark shadow-sm" onclick="toggleFavorite()"><i class="far fa-heart"></i> Mật khẩu Tủ/Bỏ thích</button>`;
        <?php else: ?>
            btnHtml += `<a href="login.php" class="btn btn-outline-warning text-dark btn-lg shadow-sm"><i class="far fa-heart"></i> Đăng nhập để lưu</a>`;
        <?php endif; ?>
        ...
        if(chapters.length > 0) {
            ...
            document.getElementById('chapter-list').innerHTML = ascChaps.map(c => `
                <div class="col-6 col-md-4 col-lg-3">
                    <a href="novel_read.php?id=${c.id}" class="text-decoration-none text-dark d-block p-2 border rounded hover-chap novel-chap-item text-truncate" title="${c.title}">${c.title}</a>
                </div>
            `).join('');
        } else {

  ...
  <div class="container">
    ...
    <div class="collapse navbar-collapse" id="navbarNav">
      ...
      <div class="d-flex position-relative me-3 my-2 my-lg-0" style="min-width: 300px;">
        <form action="search.php" method="GET" class="d-flex w-100" id="search-form" onsubmit="event.preventDefault(); window.location.href='search.php?q=' + document.getElementById('live-search-input').value;">
      ...
      <ul class="navbar-nav align-items-center" id="user-menu-area">
        ...
        <?php if(isset($_SESSION['user_id'])): ?>
            ...
            <li class="nav-item dropdown">
                ...
                <ul class="dropdown-menu dropdown-menu-end shadow">
                    <li><a class="dropdown-item" href="profile.php"><i class="fas fa-user-circle me-2"></i> Trang cá nhân</a></li>
                    
                    <?php if($_SESSION['role'] === 'admin' || $_SESSION['role'] === 'mod'): ?>
                        <li><hr class="dropdown-divider"></li>
                        <li><a class="dropdown-item text-danger fw-bold" href="admin/index.php"><i class="fas fa-cogs me-2"></i> Trang Quản Trị</a></li>
                    <?php endif; ?>
        ...
        <?php else: ?>
            ...
            <li class="nav-item">
                <a class="btn btn-warning text-dark px-3 ms-2 fw-bold" href="register.php">Đăng ký</a>
            </li>
        <?php endif; ?>
      </ul>
    </div>
...
<script>
    ...
    async function loadCategories() {
        ...
        if (novelCats && novelCats.data && novelCats.data.length > 0) {
            novelUl.innerHTML = novelCats.data.map(c => `<li><a class="dropdown-item" href="category.php?slug=${c.slug}">${c.name}</a></li>`).join('');
        } else {
        ...
        if (comicCats && comicCats.data && comicCats.data.length > 0) {
            comicUl.innerHTML = comicCats.data.map(c => `<li><a class="dropdown-item" href="comic_category.php?slug=${c.slug}">${c.name}</a></li>`).join('');
        } else {
                    ...
                    if(d && d.status === 'success' && d.data.length > 0) {
                        ...
                        d.data.forEach(group => {
                            ...
                            group.items.forEach(item => {
                                let link = group.type === 'novel' ? `novel_detail.php?id=${item.id}` : `comic_detail.php?slug=${item.slug}`;

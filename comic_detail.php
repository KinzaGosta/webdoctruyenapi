...
// --- XỬ LÝ TÌM CHƯƠNG ĐẦU VÀ CHƯƠNG MỚI NHẤT ---
$chap_start = null;
$chap_latest = null;
...
$stmt_view->bind_param("sssi", $slug, $comic_name_save, $comic_thumb_save, $random_start);
$stmt_view->execute();

$res_view = $conn->query("SELECT view_count FROM comic_views WHERE comic_slug = '$slug'");
$current_view = $res_view->fetch_assoc()['view_count'];

// 3. ĐẾM FOLLOW
$res_count = $conn->query("SELECT COUNT(*) as total FROM comic_favorites WHERE comic_slug = '$slug'")->fetch_assoc();
$total_followers = $res_count['total'];

$is_favorite = false;
if (isset($_SESSION['user_id'])) {
    $uid = $_SESSION['user_id'];
    $check = $conn->query("SELECT * FROM comic_favorites WHERE user_id = $uid AND comic_slug = '$slug'");
    if ($check->num_rows > 0) $is_favorite = true;
}
        ...
        <div class="col-md-9">
            ...
            <div class="mb-4 d-flex gap-2 flex-wrap">
                <?php if ($chap_start && $chap_latest): ?>
                    ...
                    ?>
                    <a href="comic_read.php?api=<?= $start_api ?>&name=<?= $start_name ?>&slug=<?= $slug ?>"
                    ...
                    ?>
                    <a href="comic_read.php?api=<?= $latest_api ?>&name=<?= $latest_name ?>&slug=<?= $slug ?>"
                ...
                <?php else: ?>
                    <a href="login.php" class="btn btn-outline-warning text-dark btn-lg shadow-sm"><i class="far fa-heart"></i> Đăng nhập để lưu</a>
                <?php endif; ?>
            </div>
                ...
                <div class="row g-2">
                    ...
                    ?>
                        <div class="col-6 col-md-4 col-lg-3">
                            <a href="comic_read.php?api=<?= base64_encode($chap['chapter_api_data']) ?>&name=<?= $chap_name_display ?>&slug=<?= $slug ?>"

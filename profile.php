        ...
        <div class="col-md-3">
            <div class="card shadow-sm mb-4 border-0">
                <div class="card-body text-center pt-4">
                    ...
                    <?php if ($is_owner): ?>
                        <hr>
                        <div class="d-grid"><a href="logout.php" class="btn btn-outline-danger btn-sm">Đăng xuất</a></div>
                    <?php endif; ?>
                </div>
            </div>
        </div>
                                    ...
                                    <?php while ($row = $res_novel->fetch_assoc()): ?>
                                        <div class="col-6 col-md-3">
                                            <div class="card h-100 shadow-sm border-0">
                                                <a href="novel_detail.php?id=<?= $row['id'] ?>">
                            ...
                            <?php if ($res_comic->num_rows > 0): ?>
                                <div class="row g-3">
                                    <?php while ($row = $res_comic->fetch_assoc()): ?>
                                        <div class="col-6 col-md-3">
                                            <div class="card h-100 shadow-sm border-0">
                                                <a href="comic_detail.php?slug=<?= $row['comic_slug'] ?>">
                            ...
                            <?php if ($res_comments && $res_comments->num_rows > 0): ?>
                                <div class="list-group list-group-flush">
                                    <?php while ($cmt = $res_comments->fetch_assoc()): ?>
                                        <?php
                                            $link = $cmt['novel_id'] ? "novel_detail.php?id=".$cmt['novel_id'] : "comic_detail.php?slug=".$cmt['comic_slug'];
...
function removeFav(type, idOrSlug, btn) {
    ...
    fetch('ajax_favorite.php', { method: 'POST', body: fd }).then(r => r.json()).then(data => { if(data.status === 'removed') btn.closest('.col-6').remove(); });
}
...
function removeHistory(id, btn) {
    ...
    let fd = new FormData();
    fd.append('action', 'delete_one');
    fd.append('id', id);
    fetch('ajax_history.php', { method: 'POST', body: fd }).then(r => r.json()).then(data => { if(data.status === 'success') btn.closest('.col-6').remove(); });
}
...
function deleteCommentProfile(cmtId) {
    ...
    fetch('ajax_comment.php', { method: 'POST', body: fd })
    .then(r => r.json())
    .then(data => {
...
function deleteAllHistory() {
    ...
    fetch('ajax_history.php', { method: 'POST', body: fd })
        .then(r => r.json())
        .then(data => {
            if(data.status === 'success') {
